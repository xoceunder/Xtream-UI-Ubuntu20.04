...
def configure():
    ...
    if not "/home/xtreamcodes/iptv_xtream_codes/" in open("/etc/fstab").read():
        ...
        rFile.write("tmpfs /home/xtreamcodes/content/streams tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=90% 0 0\ntmpfs /home/xtreamcodes/tmp tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=2G 0 0")
        rFile.close()
    ...
    if not os.path.exists("/etc/systemd/system/xtreamcodes.service"):
        ...
        rFile.close()
        os.system("sudo chmod +x /etc/systemd/system/xtreamcodes.service > /dev/null")
    ...
    if rAnswer.upper() == "Y":
        try:
            os.system("sudo modprobe ip_conntrack > /dev/null")
        except:
            pass
        try:
            ...
            rFile = open("/home/xtreamcodes/sysctl.on", "w")
            rFile.close()
        except:
            printc("Failed to write to sysctl file.", col.BRIGHT_RED)
    else:
        if os.path.exists("/home/xtreamcodes/sysctl.on"):
            os.remove("/home/xtreamcodes/sysctl.on")
    if not "DefaultLimitNOFILE=655350" in open("/etc/systemd/system.conf").read():
        os.system('sudo echo "\nDefaultLimitNOFILE=655350" >> "/etc/systemd/system.conf" > /dev/null')
        os.system('sudo echo "\nDefaultLimitNOFILE=655350" >> "/etc/systemd/user.conf" > /dev/null')
